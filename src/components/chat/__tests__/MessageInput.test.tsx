...
test("applies correct CSS classes based on loading state", () => {
  ...
  expect(submitButton.className).toContain("text-gray-600");
  expect(submitButton.className).toContain("hover:text-gray-900");
  ...
  expect(submitButton.className).toContain("text-gray-300");
...
test("applies pulse animation to send icon when loading", () => {
  ...
  expect(sendIcon?.getAttribute("class")).toContain("animate-pulse");
});
